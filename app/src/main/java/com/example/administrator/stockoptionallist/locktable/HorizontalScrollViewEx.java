    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (mChildIndex > mChildrenSize - 2) {
                    //因为可见的有2个
                    mChildIndex = mChildrenSize - 2;
                }
                int dx = mChildIndex * mChildWidth - scrollX;
                //控制为一次只滑动一个Children的宽度，因为每次减去的scrollX必定为Children宽度的整数倍,确定为一次要滑过几个view
//                int index = Math.round(dx / mChildWidth);
//                dx = mChildWidth * index;
                //
                smoothScrollBy(dx, 0);
